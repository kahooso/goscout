...
### Единый образец вывода CLI (task-10)
Один образец на всю программу, чтобы вывод не был мешаниной:
1. **Потоки:** данные → `stdout`, ошибки/диагностика → `stderr` (см. выше).
2. **Формат ошибки:** `<команда>: <строчными, без точки>\n`. Строчными и без точки — Go-конвенция
   ST1005 (её проверяет `staticcheck`), т.к. ошибки часто оборачивают: `outer: inner: leaf`.
3. **Кто добавляет префикс команды** (`dns`/`ports`/`http`) — тот, кто ПЕЧАТАЕТ (main/runProbe),
   через `res.Probe`/`p.Name()`. Не хардкодить имя команды в каждом `Errorf`.
4. **Контекст ошибки — через `%w`, а не имя функции.** `parsePorts()` в тексте ошибки —
   антипаттерн: имя протухает при переименовании (компилятор не заметит), и пользователю
   неинтересна внутренняя функция. Вместо этого оборачивай СМЫСЛОМ (что за ресурс/значение):
   ```go
   // низ (parsePorts): возвращает голую ошибку со смыслом, НЕ печатает, НЕ os.Exit
   return nil, fmt.Errorf("invalid port %q: %w", raw, err)   // %q кавычит — видно пустоту/пробелы
   // верх (main): добавляет имя команды при печати
   fmt.Fprintf(os.Stderr, "ports: %v\n", err)               // → ports: invalid port "abc": ...
   ```
   `%w` сохраняет цепочку для `errors.Is`/`errors.As`. Парсер-функция только ВОЗВРАЩАЕТ ошибку —
   печать и `os.Exit` делает вызывающий (разделение ответственности, fail-fast).
5. **`slog` для CLI не берём.** `slog` — структурированное логирование (level/timestamp/key-value,
   JSON) под СЕРВИСЫ и лог-агрегацию (Loki/ELK). В терминале его `time=... level=INFO` только шумит
   и ломает пайп. Уместен, если появится `--json`-режим или демон. Для CLI хватает `fmt.Fprintf`.
...
- `task-10`: **снова наступил на «флаги ДО позиционных»** (было в task-09). `ports 127.0.0.1
  --ports 80` → `flag` стопорится на `127.0.0.1`, `--ports` не парсится → `parsePorts("")` падает.
  Правило выветрилось за месяц. Лечение: продублировал в `usage` («Flags must come before…»).
- `task-10`: **сырой флаг нельзя класть в структуру напрямую.** Пытался `PortProbe{Ports: *portsRaw}`
  (`*string` → `[]uint16`) — ошибка компиляции. Между флагом и полем нужен ПАРСЕР:
  `--ports` (строка) → `parsePorts` (`[]uint16` + валидация) → `PortProbe{Ports: ...}`. И не забыть
  `targets := fs.Args()` для позиционных.
